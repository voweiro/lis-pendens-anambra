...
import { Check, Upload, AlertCircle, ChevronDown } from "lucide-react"
...
export default function UploadCaseForm() {
    ...
    const fetchJudicialDivisions = async () => {
      try {
        ...
        if (Array.isArray(divisionsData) && divisionsData.length > 0) {
          ...
          console.warn('Judicial divisions API returned empty or invalid data:', divisionsData);
    ...
    const fetchStates = async () => {
      try {
        ...
        if (Array.isArray(statesData) && statesData.length > 0) {
          ...
          console.warn('States API returned empty or invalid data:', statesData);
  ...
  const handleNext = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const missingFields = requiredFields.filter(item => !formData[item.field as keyof CaseFormData]);
    
    if (missingFields.length > 0) {
      const missingFieldNames = missingFields.map(item => item.label).join(', ');
  ...
  const handleSubmit = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      // Make the API call
      const response = await axios.post(`${baseURL}/court-staff/cases`, payload, {
        headers: {
      ...
      setError(error.response?.data?.message || error.message || 'Failed to create case');
      toast.error(error.response?.data?.message || error.message || 'Failed to create case');
    } finally {
      setLoading(false);
    }
  }
  ...
  const handleCsvUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const baseUrl = process.env.NEXT_PUBLIC_URL || '';
      const response = await axios.post(`${baseUrl}/court-staff/cases/csv`, formDataObj, {
        headers: {
          'Authorization': `Bearer ${authToken}`,
          'Accept': 'application/json',
      ...
      const errorMessage = error.response?.data?.message || error.response?.data?.errors?.csv_file || error.message || 'Failed to upload CSV';
      setError(errorMessage);
      toast.error(errorMessage);
    } finally {
      setLoading(false);
  ...
  const downloadSampleCsv = () => {
    ...
    const csvHeader = 'title,title_number,survey_plan_number,owner_name,address,parties,suit_number,court_details,date_of_commencement,date_of_disposal,nature_of_case,status,judicial_division_id';
    const sampleData = [
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      <label htmlFor="location" className="text-sm font-medium">
                        Location of Property (House or Plot No, Street..: <span className="text-red-500">*</span>
                      </label>
                      <Input 
                        id="location" 
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        <button
                          type="button"
                          className="w-full flex items-center justify-between px-3 py-2 border border-gray-300 rounded-md shadow-sm bg-white text-sm text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        >
                          <span>
                            {formData.lga || 
                             (loadingLgas ? 'Loading local governments...' : 
                              !formData.state_id ? 'Select state first' : 'Select local government')}
                          </span>
                          <ChevronDown className="h-4 w-4" />
                        </button>
                  ...
                  <div className="flex justify-between mt-6 pt-6 border-t">
                    ...
                    <a 
                      ...
                      className="inline-block px-4 py-2 rounded bg-[#23A863] hover:bg-green-600 text-white font-medium text-center"
                    >
                      Next
                    </a>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      <label htmlFor="court" className="text-sm font-medium">
                        The Court and judicial Division of the action: <span className="text-red-500">*</span>
                      </label>
                      <Input 
                        id="court" 
                    ...
                    <div className="space-y-2">
                      <label htmlFor="commencementDate" className="text-sm font-medium">
                        Date of Commencement of Claim/dispute: <span className="text-red-500">*</span>
                      </label>
                      <Input 
                        id="commencementDate" 
                        type="date" 
