...
interface  JudicialDivisionSettingProps {
  judicialdivision: string;
  onChange: (value: string) => void;
}

const JudicialDivisionSetting: React.FC<JudicialDivisionSettingProps> = ({ judicialdivision, onChange }) => {
  const [editing, setEditing] = useState(false);
  const [value, setValue] = useState(judicialdivision);
  ...
  return (
    <div className="bg-white rounded-2xl shadow p-6 flex flex-col gap-2 min-w-[320px]">
      <div className="flex justify-between items-center mb-2">
        <span className="font-semibold text-lg">Judicial Division</span>
      ...
      <div className="flex items-center gap-3 bg-gray-50 rounded-xl p-4 cursor-pointer" onClick={handleEdit}>
        <Landmark className="text-2xl text-gray-400" />
        {editing ? (
          <input
            ref={inputRef}
          ...
          <span className="text-base">{judicialdivision}</span>
        )}
      </div>
    </div>
  );
...
export default JudicialDivisionSetting;
